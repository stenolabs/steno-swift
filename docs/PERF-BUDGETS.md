...
|---|---|---|---|
| Live partial latency: provisional transcript visible < 150 ms of speech | Measured 16.32 ms worst burst (M5 Max, 26 Aug 2026): bursts of 10 paced ticks at 0/250/750/1500 finals cost 0.43/4.72/8.82/16.32 ms; 1500-final < 4x 250-final (sublinear). Budget holds. | `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer swift test --filter LivePartialLatencyTests` (in `StenoKit/`); scrape the printed `LIVE_PARTIAL_LATENCY_JSON:` line into `perf_budgets.py --json`. Heavy 1500 case drops only with `STENO_PERF_FULL=0`. | `StenoKit/Sources/StenoTranscription/LiveTranscriptFeed.swift`, harness `StenoKit/Tests/StenoTranscriptionTests/LivePartialLatencyTests.swift` |
| Final ASR real-time factor <= 0.05 RTF | Measured 0.0116 RTF (SpeechAnalyzer on macOS 26, 5 Aug 2026, six AMI-dev meetings) — see `docs/BENCH-M2-ASR.md`. Raw method in `~/Dev/sandbox/steno-diar-bench`, `RESULTS.md` section 2g. | `python3 scripts/benchmark/perf_budgets.py` scrapes the BENCH-M2 table directly; explicit payloads use key `final_asr_rtf`. Re-measure by repeating the steno-diar-bench protocol after any ASR provider change. | `StenoKit/Sources/StenoPipeline/PipelineCoordinator.swift` (final ASR stage) |
