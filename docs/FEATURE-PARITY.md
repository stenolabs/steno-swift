...
- [ ] Native Gemma inference on macOS remains unavailable.
      ...
      The production activation catalog remains deliberately empty, so unapproved pins close their filesystem capability and remain model-free, while the app process continues to link neither MLX nor Metal.
      A reviewed production checkpoint and matching resource limits, user-facing import and provider-selection UI, explicit orphan and corrupt-install recovery, production Hardened Runtime validation, a real model run, and negative signed abuse or multiprocess XPC coverage remain open.
