...
- [ ] Native Gemma inference on macOS remains unavailable.
      ...
      Recovery never auto-publishes, and an explicit user-authorized repair flow for retained corrupt targets remains open alongside a reviewed production checkpoint and matching resource limits in all three catalogs, user-facing import and provider-selection UI, a real model run, and negative signed abuse or multiprocess XPC coverage.
