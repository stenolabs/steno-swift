...
- [ ] Native Gemma inference on macOS remains unavailable.
      ...
      A reviewed production checkpoint, user-facing import UI, explicit orphan and corrupt-install recovery, production Hardened Runtime validation, an immutable child-file activation boundary, provider activation, a real model run, and negative signed abuse or multiprocess XPC coverage remain open.
