...
extension MLXLanguageModel {
    ...
    public enum Availability: Sendable, Equatable {
        ...
        /// launch while Metal shaders are JIT-compiled. Deferred models can use
        /// ``MLXLanguageModel/Executor/prewarm(model:transcript:)`` (via
        /// `session.prewarm()`) to amortize that cost ahead of time. For stored
        /// containers, `session.prewarm()` is intentionally a no-op; the owner
        /// explicitly awaits ``MLXLanguageModel/warmUp()`` instead.
        case available

        /// Weights are actively being fetched.
        ///
        /// For deferred models, this corresponds to a genuine in-flight download (an
        ...
        /// determine the resulting state. Stored containers never enter this
        /// state because they have no downloader or deferred cache entry.
        case downloading
    ...
    public var availability: Availability {
        get async {
            ...
            // A stored container has already been supplied by its owner. Its
            // availability has no disk, downloader, or global-cache state.
    ...
    public var freeDiskSpaceBytes: Int64? {
        guard let weightsLocation else { return nil }
    ...
    func modelExistsOnDisk() -> Bool {
        // A stored container has no filesystem-backed source. Treat it as
        // present without consulting a path, so direct internal callers retain
        // a safe predicate rather than crashing on an absent resolver.
        guard let weightsLocation else { return true }
