...
8. Native Gemma 4 app integration: `GemmaService` establishes a macOS 27 compile and local-verification boundary around an MLX runtime that conforms to Apple's Foundation Models `LanguageModel` API.
   ...
   The dependency-free IPC boundary and a real model-free XPC integration test are implemented, while production caller authentication, the recording quiescence barrier, bounded request lifecycle management, the model installer or importer, and verified model-runtime activation remain unresolved.
